...
*/
bool IsFIndex(string fileName) {
	return fileName.find(".findex") != fileName.npos;
}
...
bool ParseFIndex(string fileName, map <string, vector<int> > & holeNumbers) {
	...
	while ( getline(fIndex, line) ) {
		...
		string fileName;
		int holeNumber;
		lineStrm >> fileName >> holeNumber;
		holeNumbers[fileName].push_back(holeNumber);
	}
}
	


bool GetNext(HDFBasReader &smrtReader, SMRTSequence &read, bool useAll, vector<int> &holeNumbers, int &curHoleNumber) { 
	
	if (useAll) {
		return smrtReader.GetNext(read);
	}
	else {
		if (curHoleNumber >= holeNumbers.size()) {
			return false;
		}
		else {
			smrtReader.GetReadAt(holeNumbers[curHoleNumber], read);
			curHoleNumber++;
			return true;
		}
	}
...
int main(int argc, char* argv[]) {
  ...
  clp.ParseCommandLine(argc, argv);
	...
	if (printCcs == true and holeNumbers.size() > 0) {
		cout << "ERROR! Random access is not yet implemented for CCS printing." << endl;
		exit(1);
	}
  ...
  if (printFastq) {
	...
	map<string, vector<int> > fIndex;
	bool useAll;
	if (FileOfFileNames::IsFOFN(plsFileName)) {
		FileOfFileNames::FOFNToList(plsFileName, plsFileNames);
		useAll = holeNumbers.size() == 0;
	}
	else if (IsFIndex(plsFileName)) {
		ParseFIndex(plsFileName, fIndex);
		map<string, vector<int> >::iterator fIndexIt;
		for (fIndexIt = fIndex.begin(); fIndexIt != fIndex.end(); ++fIndexIt) {
			plsFileNames.push_back(fIndexIt->first);
		}
		useAll = false;
	}
	else {
		// 
		// Just operating on a single file.
		plsFileNames.push_back(plsFileName);
		// Store the hole numbers for this file
		fIndex[plsFileName] = holeNumbers;
		useAll = holeNumbers.size() == 0;
	}
	if (regionsFOFNName == "") {
  ...
  sort(holeNumbers.begin(), holeNumbers.end());
	...
	for (plsFileIndex = 0; plsFileIndex < plsFileNames.size(); plsFileIndex++) {
		//
		// Find out which zmw's are going to be extracted for this file.
		//
		if (fIndex.find(plsFileNames[plsFileIndex]) != fIndex.end()) {
			holeNumbers = fIndex[plsFileNames[plsFileIndex]];
		}
		else {
			holeNumbers = vector<int>();
		}
    ...
    HDFCCSReader<CCSSequence> ccsReader;		
		...
		if (printCcs) {
			...
			if (holeNumbers.size() > 0) {
				smrtReader.PrepareForRandomAccess();
			}
		}
    ...
    CCSSequence  ccsSeq;
		...
		while (true) {
			if (printCcs == true or printOnlyBest == true) {
				...
				if (GetNext(smrtReader, seq, useAll, holeNumbers, curHoleNumber) == false) {
					break;
				}
			}
